<template>
  <Header></Header>
  <div class="historys">
    <div class="history" :key="index" v-for="(history, index) in historys">
      ...
      <p class="store">{{ history.nameStore }}</p>
      <p class="date">{{ formatDate(history.datePurchase) }}</p>
      <p class="price">{{ formatValue(history.price, history.quantity) }}</p>
      <button
        class="button-function"
        v-if="history.visible"
        @click="$router.push(`book/${history.idBook}`)"
      >
        Comprar Novamente
      </button>
...
import { getHistorys } from "@/service/history/";
import Header from "@/components/HeaderComp.vue";
import { DTOhistory } from "@/service/history/type";
...
export default defineComponent({
  ...
  methods: {
    ...
    formatValue(price: number, quantiy: number): string {
      return `${this.formatPrice(price / quantiy)} x ${quantiy} = ${this.formatPrice(price)}`;
    },
...
.backgroundSize::-webkit-scrollbar {
  width: 0;
  height: 0;
  opacity: 0;
}
