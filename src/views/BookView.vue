<template>
  ...
  <div class="book">
    ...
    <div class="book-info">
      ...
      <span id="info">
        <h2 id="author">{{ bookData.author }}</h2>
        <h3 id="store">
          {{ bookData.storeName }} - {{ bookData.genre }} - {{ bookData.publicationYear }}
        </h3>
      </span>
      <span id="price" @click="$router.push(`/buy/${$route.params.uuid}`)">{{
...
import { getByUUID } from "@/service/books";
import { DTOBooksMounted } from "@/service/books/type";
...
export default defineComponent({
  ...
  async mounted() {
    if (typeof this.$route.params.uuid === "string" && this.$route.params.uuid !== "") {
      const result = await getByUUID(this.$route.params.uuid);
      if (result.status) {
        if (result.data) {
          this.bookData = result.data;
          this.priceFinal = this.bookData.price;
        }
      } else {
...
#price:hover {
  ...
  box-shadow: 0px 0px 20px 10px rgba(255, 230, 0, 0.4);
}
</style>
